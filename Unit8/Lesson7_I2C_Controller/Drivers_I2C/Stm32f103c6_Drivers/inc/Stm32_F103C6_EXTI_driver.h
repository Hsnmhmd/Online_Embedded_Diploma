...
typedef struct{
	...
	uint8_t						Trigger_Case;			//SpecifIes The Triggering Edge For EXTI
														//Set Based on @ref EXTI_Trigger_define
	uint8_t						IRQ_EN;					//Enable or Disable EXTI(Concerns IRQ mask
														//in EXTI and In NVIC)
														//Set Based on @ref EXTI_IRQ_define
	void (*P_IRQ_CALLBACK)(void)	;					//Set Function That Will be called once IRQ Happen
